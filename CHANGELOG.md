...
### Fixed
- **Lint pipeline**: Removed unused functions `createHoleFromPlunge` and `createHoleFromDrillCycle` from analyzer to fix golangci-lint failures

## [1.0.2] - 2026-02-09

### Added
- **Peck drilling consolidation**: Multiple peck cycles at the same XY now merge into a single hole with peck metadata
- **Peck drilling assertions**: `IsPeckDrilled()` and `WithPeckCount()` to validate peck operations
- **New examples**: peck drilling, multi-tool jobs, contours with arcs, warning handling, and tolerance usage

### Changed
- **Depth calculation**: Hole depth is now absolute (`WorkpieceTopZ - BottomZ`) instead of travel distance from safety height
- **Analyzer config**: Added `ConsolidatePeckDrilling` and `PeckDetectionRadius` options
- **Documentation**: Updated API examples to match v1.0.1+ and clarified depth convention
...
[1.0.3]: https://github.com/eduardotorresdev/gocode-check/releases/tag/v1.0.3
[1.0.2]: https://github.com/eduardotorresdev/gocode-check/releases/tag/v1.0.2
